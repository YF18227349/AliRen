...
layui.define(['table', 'jquery', 'form', 'laydate'], function (exports) {
    ...
    tableFilter.render = function (opt) {
        ...
        var elem = $(opt.elem || '#table'),
            ...
            tableObj = opt.tableObj,
            mode = opt.mode || "local";
        opt.done = opt.done || function () {
        };
        elemId = tableObj.id;
        //写入默认缓存
        tableFilter.cache[elemId] = tableObj.where;
        if (typeof (tableObj.where.where) == 'undefined') {
            tableFilter.cache[elemId] = tableObj.where;
        } else {
            tableFilter.cache[elemId] = tableObj.where.where;
        }
        //主运行
        var main = function () {
            //默认过滤
            ...
            layui.each(filters, function (i, filter) {
                ...
                if (tableFilter.cache[elemId][filterName]) {
                    ...
                    filterIcon.siblings("span").addClass('fc-red');
                } else {
                    filterIcon.removeClass("tableFilter-has")
                    filterIcon.siblings("span").removeClass('fc-red');
                }
                //图标点击事件
                filterIcon.on("click", function (e) {
                    ...
                    form.on('submit(tableFilter)', function (data) {
                        ...
                        let getConnector = function (filters, filterName, filterType) {
                            let connector = '';
                            for (let p in filters) {//遍历json数组时，这么写p为索引，0,1
                                let v = filters[p];
                                if (v.field == filterName) {
                        ...
                        //过滤项写入缓存
                        tableFilter.cache[elemId][filterName] = data.field[filterName] + ':::' + getConnector(filters, filterName, filterType);
    ...
    layui.link(layui.cache.base + 'extend/lib/tableFilter/tableFilter.css?v=2');
    //输出接口
